...
execute unless block ~ ~ ~ minecraft:air run return run execute if block ~ ~ ~ #noodles:turnable positioned ~ ~-.5 ~ as @e[type=minecraft:marker,tag=tile,limit=1,sort=nearest] at @s run function noodles:level/tile/rotate/init
